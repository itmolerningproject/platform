...
def save_services_to_csv(current_dateTime, services):
    with open('services.csv', mode='a', newline='') as csv_file:
        csv_writer = csv.writer(csv_file, delimiter=',')
        fieldnames = ['NAME', 'CPU %', 'MEM USAGE / LIMIT / MEM %', 'NET I/O', 'BLOCK I/O', 'STATUS', 'Date/Time']

        # Если файл пустой, записываем заголовки
        if os.path.getsize('services.csv') == 0:
        ...
        for service in services:
            row = []
            row.append(service.get("NAME"))
            row.append(service.get("CPU %") )
            row.append( service.get("MEM USAGE / LIMIT / MEM %") )
            row.append(service.get("NET I/O") )
            row.append(service.get("BLOCK I/O") )
            row.append(service.get("STATUS"))
            row.append(current_dateTime)
            rows.append(row)
        csv_writer.writerows(rows)

def save_pc_to_csv(current_dateTime, cpu_percent, memory_percent, disk_percent):  
    with open('pc.csv', mode='a', newline='') as csv_file:
        fieldnames = ['CPU %', 'Memory %', 'Memory mb', 'Memory max','Disk %', 'Date/Time']
        csv_writer = csv.writer(csv_file, delimiter=',')

        # Если файл пустой, записываем заголовки
        if os.path.getsize('pc.csv') == 0:
           csv_writer.writerow(fieldnames)

        # Записываем данные в файл
        row = []
        row.append(cpu_percent)
        row.append(memory_percent.percent)
        row.append(memory_percent.used) 
        row.append(memory_percent.available) 
        row.append(disk_percent)
        row.append(current_dateTime)
...
def send_stats(update: Update, context: CallbackContext):
    ...
    save_services_to_csv(current_dateTime, services)
    save_pc_to_csv(current_dateTime, cpu_percent, memory_info, disk_usage.percent)
